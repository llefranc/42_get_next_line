...
/*   Updated: 2020/11/25 12:11:02 by lucaslefran      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		initchain(t_list **first, t_list *fdelem, char **line, int fd)
{
	...
	if (!(*first))
	{
		if (!read(fd, buffer, BUFFER_SIZE))
			return (0);
		*first = ft_lstnew(fd, ft_strdup(buffer, BUFFER_SIZE));
		return (1);
	}
	...
	if (!read(fd, buffer, BUFFER_SIZE))
		return (0);
	fdelem->next = ft_lstnew(fd, ft_strdup(buffer, BUFFER_SIZE));
	return (1);
}
...
int		findline(t_list *elem, char **line, size_t len)
{
	...
	while (1)
	{
		...
		while (elem->content[len] != '\n' && elem->content[len] != '\0')
			len++;
		if (!(*line = ft_strjoin(*line, ft_substr(elem->content, 0, len))))
			return (-1);
		if (elem->content[len] == '\n')
		{
			if (!(tmp = ft_substr(elem->content, len + 1, BUFFER_SIZE)))
				return (-1);
			free(elem->content);
			elem->content = tmp;
			return (1);
		...
		{
			ft_memset(elem->content, 0, BUFFER_SIZE + 1);
